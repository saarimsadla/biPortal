...
if ($osName === "Windows") {
	$pythonCmd = "python";
	$scriptPath = realpath(__DIR__ . "\\..\\pythonScripts\\mongodb_query.py");
	$operatingDir = realpath(__DIR__ . "\\..\\pythonScripts\\"); 
} else {
	$pythonCmd = "python";
	$scriptPath = realpath(__DIR__ . "/../pythonScripts/mongodb_query.py");
	$operatingDir = realpath(__DIR__ . "/../pythonScripts/");
...
function getWlData($valueToGet){
	...
	$initial_output = shell_exec("$command 2>&1");
	//print_r("ss ".$command);
	//print_r("wer: ".$initial_output);
...
$un = isset($_GET['user']) ? $_GET['user'] : null;
$pss = isset($_GET['pass']) ? $_GET['pass'] : null;
...
/*
 &&
    $un === "a7F9xP2qL8vZ3mN0rT6wY1bC5eKdUjXh" &&
    $pss === "Z4rV8nW1sQpLmT7cXy9aEbKjMfDoHg2C"
*/
if (
    $un !== null &&
    $pss !== null &&
...
$iniCommand = $pythonCmd. ' ' .$operatingDir .'\\authConfirm.py '. escapeshellarg($user) . ' ' . escapeshellarg($pass);

//print_r("ww ".$iniCommand);

$result = shell_exec($iniCommand);
...
if (strpos($result, 'User not found') !== false){
	...
	if ($data !== null) {
		...
		//echo "Def whitelabel data array: $whiteLabelARR\n";
